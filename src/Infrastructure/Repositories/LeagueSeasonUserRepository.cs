...
{
    public async Task<IReadOnlyCollection<UserSummaryDto>> GetUsersByLeagueSeasonIdAsync(Guid leagueSeasonId)
        => await DbSet
            .Where(lsu => lsu.LeagueSeasonId == leagueSeasonId)
            .OrderBy(lsu => lsu.User.FullName)
            .Select(lsu => new UserSummaryDto(
                lsu.User.Id,
                lsu.User.FullName,
                lsu.User.WcaId,
                lsu.User.IsAdmin,
                lsu.User.AvatarUrl,
                lsu.User.AvatarThumbnailUrl))
            .ToListAsync();
